...
export const GamesView: React.FC = () => {
  ...
  useEffect(() => {
    if (gameSettings.filword_mode === 'synced' && gameSettings.filword_unlocked) {
  ...
  useSmartPolling(
    fetchFilwordLobbyCount,
    3000,
    gameSettings.filword_mode === 'synced' && gameSettings.filword_unlocked
  );

  useEffect(() => {
    if (gameSettings.quiz_mode === 'synced' && gameSettings.quiz_unlocked) {
      fetchQuizLobbyCount();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useSmartPolling(
    fetchQuizLobbyCount,
    3000,
    gameSettings.quiz_mode === 'synced' && gameSettings.quiz_unlocked
  );
  ...
  useSmartPolling(() => fetchGameSettings(false), 5000);
