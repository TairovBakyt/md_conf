...
export const AdminChatView: React.FC = () => {
  ...
  const othersWithUnread = selectedOther
    ? inbox.filter((item) => item.otherId !== selectedOther.id && item.unreadCount > 0)
    : [];
        ...
        return (
          <div
            ...
            className={`max-w-[80%] w-fit rounded-lg text-sm relative cursor-pointer select-none flex items-start gap-2 ${
              ...
              isMediaOnly ? 'bg-transparent' : isMine ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-200'
            } ${isMine ? 'self-end' : 'self-start'} ${isSelected ? 'ring-2 ring-amber-400' : ''}`}
          >
            {msgSelectionMode && (
              <input
                type="checkbox"
        ...
        <div className="md:hidden fixed inset-0 z-50 bg-slate-900 flex flex-col">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 shrink-0">
            ...
            <div className="flex items-center gap-1.5 min-w-0 mx-2">
              <span className="text-slate-100 text-sm font-medium truncate">{selectedOther.username}</span>
