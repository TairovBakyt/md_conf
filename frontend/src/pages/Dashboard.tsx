...
export const Dashboard: React.FC = () => {
  ...
  // PIN, который система изменила при регистрации (выбранный был занят).
  // Показываем один раз, иначе человек запомнит не тот PIN.
  const [newPin, setNewPin] = useState<string | null>(() =>
    sessionStorage.getItem("mdconf_new_pin")
  );

  const dismissNewPin = () => {
    sessionStorage.removeItem("mdconf_new_pin");
    setNewPin(null);
  };
  ...
  const fetchProfile = async (isInitial = false) => {
    ...
    try {
      ...
      if (settingsRes.ok) {
        ...
        if (
          settingsData.quiz_unlocked &&
          settingsData.quiz_mode === "synced" &&
          !data.is_quiz_passed
        ) {
          navigate("/quiz");
          return;
        ...
        if (
          settingsData.filword_unlocked &&
          settingsData.filword_mode === "synced" &&
          !data.is_filword_passed
        ) {
          navigate("/filword");
          return;
        }
        ...
        */}
        ...
        <div className="flex-1 flex flex-col items-center min-w-0">
          <div className="w-full max-w-md">
                        {newPin && (
              <div className="w-full bg-amber-950/60 border border-amber-500/40 rounded-2xl p-4 mb-4">
                ...
                <p className="text-amber-200/70 text-xs leading-relaxed mb-3">
                  Выбранный вами PIN был уже занят, поэтому система добавила к
                  нему символ. Вводите его целиком при следующем входе — он
                  всегда доступен в разделе «Мой QR».
                </p>
                <button
                  onClick={dismissNewPin}
                  className="text-amber-300 hover:text-amber-100 text-xs underline transition-colors"
                >
                  Понятно, запомнил
                </button>
              </div>
            ...
            <div className="w-full bg-gradient-to-r from-indigo-600 to-cyan-600 rounded-2xl p-4 mb-4 text-center">
              <p className="text-white text-sm font-medium leading-relaxed">
                🎁 Сегодня вы можете получить призы за прохождение игровых
                станций. Чем больше баллов наберёте — тем лучше подарок сможете
                получить.
              </p>
            </div>
            ...
            {profile.is_quiz_passed ? (
              ...
              <div className="w-full mt-3 p-4 bg-indigo-950/40 border border-indigo-500/30 rounded-2xl flex items-center justify-center gap-2 text-indigo-300 text-sm">
                ⏳ Викторина сейчас начнётся у всех одновременно — держите
                приложение открытым
              </div>
            ) : gameSettings.quiz_unlocked ? (
              <button
            ...
            {profile.is_filword_passed ? (
              ...
              gameSettings.filword_mode === "synced" ? (
              <div className="w-full mt-3 p-4 bg-indigo-950/40 border border-indigo-500/30 rounded-2xl flex items-center justify-center gap-2 text-indigo-300 text-sm">
                ⏳ Филворд сейчас начнётся у всех одновременно — держите
                приложение открытым
              </div>
            ) : gameSettings.filword_unlocked ? (
              <button
        ...
        <div className="flex xl:hidden flex-col gap-4 w-full">
          ...
          <StationProgress
            userId={profile.id}
            isQuizPassed={profile.is_quiz_passed}
          />
